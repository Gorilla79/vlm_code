...
class ASFMNavigator:
    ...
    def detect_humans(self, color_frame, depth_frame):
        # ✅ YOLO용 이미지 (정규화)
        color_image_yolo = np.asanyarray(color_frame.get_data()).astype(np.float16) / 255.0
        input_tensor = torch.from_numpy(color_image_yolo).permute(2, 0, 1).unsqueeze(0).to(self.device).half()
    ...
    def visualize(self, frame, humans, move_direction, speed, vlm_action=None, vlm_speed=None):
        ...
        # ✅ VLM 결과 표시 (상단 노란색)
        if vlm_action is not None and vlm_speed is not None:
            vlm_text = f"VLM Suggestion: {vlm_action} at {vlm_speed:.2f} m/s"
            cv2.putText(frame, vlm_text, (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)  # Yellow Color
    ...
    def run(self):
        try:
            while True:
                ...
                color_image_yolo = np.asanyarray(color_frame.get_data()).astype(np.float16) / 255.0
                # ✅ VLM용 원본 이미지 (정규화 X)
                color_image_vlm = np.asanyarray(color_frame.get_data())
                # ✅ Depth 이미지 (단위 변환: mm → meters)
                depth_image = np.asanyarray(depth_frame.get_data()).astype(np.float32) / 1000.0  

                # 사람 감지 및 추적
                humans = self.detect_humans(color_frame, depth_frame)
                ...
                if len(humans) >= 3:
                    # ✅ VLM 모델로 명령 생성
                    action, speed = self.generate_vlm_command(color_image_vlm, situation_text)
                    move_direction = {
                ...
                # 결과 시각화
                display = self.visualize(color_image_vlm, humans, move_direction, speed)
